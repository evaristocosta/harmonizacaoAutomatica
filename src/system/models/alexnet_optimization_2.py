...
def model(hp, l2_reg=0.0):
    ...
    neuronios_cnn = hp.Choice("neurons", [32, 64, 128])
    dropout = hp.Choice("dropout", [0.10, 0.25])
    ...
    # Layer 3, 4 e 5
    for i in range(hp.Int("cnn_layers", 3, 4)):
    ...
    # Layer 7
    alexnet.add(Dense(hp.Choice("dense_2", [128, 256, 512, 1024])))
    alexnet.add(BatchNormalization())
    alexnet.add(Activation(ativacao))
